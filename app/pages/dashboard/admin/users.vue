...
const { data: organizations, pending: loadingOrgs } = await useApi<Organization[]>('/api/admin/organizations')
...
const { data: usersData, refresh, pending: loadingUsers, error: usersError } = await useAsyncData<UsersDataResult>(
  'admin-users-list',
  async () => {
    try {
...
const removeUserFromOrg = async (orgId: string) => {
    ...
    try {
        ...
        if (userToManage.value) {
            userToManage.value.organizations = userToManage.value.organizations.filter((o: any) => o.id !== orgId)
        }
    } catch (e: any) {
  ...
  <div class="flex flex-col gap-4 h-full">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div class="flex items-center gap-2">
        ...
        <span v-if="normalizedUsers.length" class="text-gray-500 text-sm">({{ normalizedUsers.length }})</span>
      </div>
        ...
        >
          <template #default="{ modelValue }">
              <span class="flex items-center gap-2">
                 <UAvatar v-if="modelValue?.logo" :src="modelValue.logo" :alt="modelValue.name" size="2xs" />
    ...
    <!-- Pagination -->
    <div v-if="totalUsers > limit" class="flex justify-center border-t border-gray-200 dark:border-gray-800 pt-4">
        <UPagination v-model="page" :total="totalUsers" :page-size="limit" />
    </div>
    
    <!-- Ban Modal -->
    <UModal v-model:open="isBanModalOpen" title="Ban User" description="Prevent this user from accessing the platform.">
      <template #body>
           ...
           <div class="flex flex-col gap-6">
               ...
               <div class="flex flex-col gap-4">
                   ...
                   <div v-if="userToManage?.organizations?.length" class="flex flex-col gap-3">
                       <div v-for="org in userToManage.organizations" :key="org.id" class="p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 flex flex-col gap-3">
                           <div class="flex items-center justify-between">
                               <div class="flex items-center gap-2">
                                   <UAvatar :src="org.logo" :alt="org.name" size="2xs" />
                                   <div class="flex flex-col">
                                       <span class="text-sm font-bold">{{ org.name }}</span>
                                       <span class="text-[10px] uppercase font-bold text-gray-400">{{ org.role || 'Member' }}</span>
                                   </div>
                               </div>
                               <UButton icon="lucide:user-minus" color="error" variant="ghost" size="xs" @click="removeUserFromOrg(org.id)" />
                           </div>

                           <!-- Teams for this Org -->
                           <div class="flex flex-col gap-2 pl-6 border-l-2 border-gray-200 dark:border-gray-700">
                               <div class="flex items-center gap-1 text-[10px] uppercase font-bold text-gray-400">
                                   <UIcon name="lucide:users" class="size-3" />
                                   Teams
                               </div>
                               
                               <div v-for="team in userToManage.teams.filter(t => t.organizationId === org.id)" :key="team.id" class="flex items-center justify-between py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                                   <span class="text-xs text-gray-600 dark:text-gray-400 capitalize">{{ team.name }}</span>
                                   <UButton icon="lucide:x" color="error" variant="ghost" size="xs" @click="removeUserFromTeam(team.id)" />
                               </div>
                               <div v-if="!userToManage.teams.filter(t => t.organizationId === org.id).length" class="text-[10px] text-gray-400 italic">No teams in this organization.</div>
                               ...
                               <div class="flex items-center gap-2 mt-1">
                                   <USelectMenu 
                                      ...
                                      class="flex-1"
                                   />
                                   <UButton icon="lucide:plus" size="xs" color="neutral" @click="addUserToTeam(org.id)" :disabled="!addToTeamStates[org.id]" />
                               </div>
                           </div>
                       </div>
                   </div>
                   ...
                   <div class="flex items-end gap-2 pt-4 border-t border-gray-100 dark:border-gray-800">
                       <div class="flex-1 flex flex-col gap-1">
                           <span class="text-[10px] uppercase font-bold text-gray-400">Join New Organization</span>
                           <USelectMenu
                       ...
                       <div class="w-24 flex flex-col gap-1">
                           ...
                           <USelect v-model="addToOrgState.role" :items="['member', 'admin', 'owner']" size="sm" class="w-full" />
                       </div>
                       <UButton icon="lucide:plus" color="primary" @click="addUserToOrg" :disabled="!addToOrgState.org" />
                   </div>
               </div>
               ...
               >
                  <template #description>
                      <div class="flex flex-col gap-3 mt-2">
                          <p class="text-xs opacity-90">Actions here can significantly impact the user's account access.</p>
                          <UButton 
                             ...
                             variant="outline" 
                             class="w-full"
                             @click="userToManage?.user?.banned ? unbanUser(userToManage.userId) : openBanModal(userToManage)" 
                          />
                      </div>
                  </template>
               </UAlert>
