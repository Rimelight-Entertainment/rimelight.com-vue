...
<template>
  <div class="flex flex-col gap-xl">
    ...
    <UEmpty v-if="categories.length === 0" title="No exercise categories added yet." description="Start your workout plan!" :actions="[ { icon: 'lucide:plus', label: 'Add Category', onClick: addCategory }]"/>
    ...
    <template v-else>
            ...
            <template #item="{ element: category }">
                <UCard variant="soft" :ui="{ body: 'flex flex-col gap-md' }">
                        ...
                        <template #item="{ element: exercise }">
                            <div class="flex flex-col gap-md group">
                                ...
                                <div class="flex items-start gap-sm">
                                    ...
                                    <UButton color="error" variant="ghost" icon="i-lucide-trash" size="sm" class="opacity-0 group-hover:opacity-100 transition-opacity" @click="removeExercise(category.id, exercise.id)" />
                                </div>

                                <!-- Sets List -->
                                <div class="flex flex-col gap-sm pl-8">
                                    <div class="grid grid-cols-12 gap-2 text-[10px] font-bold text-muted uppercase tracking-wider px-2">
                                    ...
                                    <div v-for="(set, index) in exercise.sets" :key="set.id" class="grid grid-cols-12 gap-2 items-center bg-dimmed p-2 rounded-lg border border-muted relative group/set">
                                        <div class="col-span-1 flex items-center justify-center font-mono text-sm text-muted">
                                            {{ index + 1 }}
                                        </div>
                                        <div class="col-span-3">
                                            <UInput v-model="set.kg" size="xs" placeholder="kg" variant="soft" @change="onChange">
                                                <template #trailing><span class="text-muted text-[10px]">KG</span></template>
                                            </UInput>
                                        </div>
                                        <div class="col-span-3">
                                            <UInput v-model="set.reps" size="xs" placeholder="reps" variant="soft" @change="onChange">
                                                 <template #trailing><span class="text-muted text-[10px]">REPS</span></template>
                                            </UInput>
                                        </div>
                                        <div class="col-span-3">
                                            <UInput v-model.number="set.restTimer" size="xs" type="number" variant="soft" placeholder="60s" @change="onChange">
                                                 <template #trailing><span class="text-muted text-[10px]">SEC</span></template>
                                            </UInput>
                                        </div>
                                    ...
                                    <UButton icon="i-lucide-plus" label="Add Set" size="xs" variant="ghost" color="neutral" @click="addSet(category.id, exercise.id)" />
                                </div>
                            </div>
                        </template>
                    </draggable>
                    ...
                    <div class="flex justify-start">
                        <UButton icon="i-lucide-plus" size="sm" variant="ghost" color="neutral" label="Add Exercise" @click="addExercise(category.id)" />
                    </div>
                </UCard>
            </template>
        </draggable>
        ...
        <div class="flex justify-start">
            <UButton icon="i-lucide-plus" size="sm" color="neutral" variant="soft" label="Add Category" @click="addCategory" />
        </div>
    </template>
  </div>
</template>
