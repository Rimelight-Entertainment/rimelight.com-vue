...
const currentStretchIndex = ref<number>(-1);
const timeLeft = ref(0);
const timerInterval = ref<any>(null);

const allStretches = computed(() => {
    return categories.value.flatMap(cat => cat.items.map(item => ({ ...item, categoryName: cat.name })));
});
...
function startCurrentStretch() {
    ...
    // Play sound if not first (or first? Usually transition sound is good. Let's play start sound)
    playSound();
...
<template>
  <div class="flex flex-col gap-xl">
    ...
    <div v-if="isRunning && currentStretch" class="sticky top-4 z-20 bg-elevated text-highlighted rounded-xl p-md shadow-xl">
        <div class="flex items-start justify-between">
          <div class="flex flex-col gap-xs">
             <div class="text-sm">
                {{ currentStretch.categoryName }} • {{ currentStretchIndex + 1 }}/{{ allStretches.length }}
            </div>
    ...
    <UEmpty v-if="categories.length === 0" title="No stretch categories found." description="Create one to get started." :actions="[ { icon: 'lucide:plus', label: 'Add Category', onClick: addCategory }]"/>
              ...
              <UCard variant="soft" :ui="{ body: 'flex flex-col gap-md' }">
                ...
                >
                          <template #item="{ element: stretch }">
                              ...
                              >
                                  <UButton size="sm" variant="ghost" icon="lucide:grip-vertical" class="drag-handle" />
                                  ...
                                  <div class="w-24 flex flex-row gap-xs items-center">
                                      <UInputNumber v-model="stretch.duration" orientation="vertical" size="sm" variant="soft" @change="onChange" />
                                    <span class="text-sm text-muted">s</span>
                                  </div>

                                <UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" class="opacity-0 group-hover:opacity-100 transition-opacity" @click="removeStretch(category.id, stretch.id)" />
                              </div>
                          </template>
                    </draggable>

                <UButton icon="lucide:plus" size="sm" variant="ghost" label="Add Stretch" @click="addStretch(category.id)" />
              </UCard>
          </template>
      </draggable>
