      ...
      <UDashboardNavbar title="Grocery List">
        ...
        <template #right>
            <div class="flex items-center gap-3">
                ...
                <UButton label="Manage Stores" icon="i-lucide-store" color="neutral" variant="ghost" size="sm" @click="isStoreModalOpen = true" />
    ...
    <template #body>
      <div class="flex flex-col gap-6">
          ...
          <div class="flex items-center gap-2 max-w-md">
              <UInput v-model="newCategoryInput" placeholder="New Category (e.g. Fridge, Pantry)..." class="flex-1" @keyup.enter="addCategory" />
              <UButton label="Add" icon="i-lucide-plus" color="neutral" variant="soft" @click="addCategory" />
          </div>

          <!-- Empty State -->
          <div v-if="localCategories.length === 0" class="flex flex-col items-center justify-center py-20 text-gray-400">
              ...
              >
                  <template #item="{ element: category }">
                      <UCard class="flex flex-col h-full group/card relative overflow-visible">
                          ...
                          <div class="flex-1 space-y-2">
                              ...
                              >
                                  <template #item="{ element: item }">
                                      <div v-if="!filterToBuy || !item.isBought" class="group/item flex items-center justify-between p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-transparent hover:border-primary-500/30 transition-all" :class="{'opacity-50 grayscale': item.isBought}">
                                          <div class="flex items-center gap-3 overflow-hidden">
                                              <UCheckbox :model-value="item.isBought" @update:model-value="toggleBought(category.id, item.id)" />
                                              <div class="flex flex-col overflow-hidden">
                                                  <span class="font-medium text-sm truncate" :class="{'line-through': item.isBought}">{{ item.name }}</span>
                                                  <div class="flex items-center gap-2 text-[10px] text-gray-500">
                                                      <UBadge :label="item.stockStatus.replace('-', ' ')" :color="getStockColor(item.stockStatus)" size="xs" variant="soft" class="capitalize" />
                                                      <span>• {{ item.amount }} {{ item.unit || '' }}</span>
                                                      <span v-if="item.lastPrice" class="text-primary-500 font-semibold">${{ item.lastPrice }}</span>
                                                      <span v-if="item.storeId" class="truncate opacity-70">• {{ getStoreName(item.storeId) }}</span>
                                                  </div>
                                              </div>
                                          </div>
                                          <div class="flex items-center gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
                                              <UButton icon="i-lucide-edit-3" color="neutral" variant="ghost" size="xs" @click="openEditModal(category.id, item)" />
                          ...
                          <template #footer>
                              <div class="flex items-center gap-2">
                                  <UInput v-model="newItemName[category.id]" placeholder="Add item..." size="xs" class="flex-1" @keyup.enter="quickAddItem(category.id)" />
                                  <UButton icon="i-lucide-plus" size="xs" color="neutral" variant="ghost" @click="quickAddItem(category.id)" />
                                  <UButton icon="i-lucide-file-text-plus" size="xs" color="neutral" variant="ghost" @click="openAddDetailedModal(category.id)" />
                              </div>
                          </template>
                      </UCard>
                  </template>
      ...
      <!-- Item Detail Modal -->
      <UModal v-model="isItemModalOpen" :title="isEditing ? 'Edit Item' : 'Add Detailed Item'">
          <template #body>
            <div class="space-y-4">
                <UFormField label="Item Name" required>
                    <UInput v-model="editingItem.name" placeholder="Milk, Eggs..." />
                </UFormField>

                <div class="grid grid-cols-2 gap-4">
                    <UFormField label="Amount">
                        <UInput v-model="editingItem.amount" type="number" />
                    </UFormField>
                    <UFormField label="Unit">
                        <UInput v-model="editingItem.unit" placeholder="kg, lit, pack..." />
                    </UFormField>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <UFormField label="Brand">
                        <UInput v-model="editingItem.brand" placeholder="Brand name..." />
                    </UFormField>
                    <UFormField label="Last Price">
                        <UInput v-model="editingItem.lastPrice" type="number" step="0.01" icon="i-lucide-dollar-sign" />
                    </UFormField>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <UFormField label="Preferred Store">
                        <USelect v-model="editingItem.storeId" :items="storeOptions" />
                    </UFormField>
                    <UFormField label="Stock Status">
                        <USelect v-model="editingItem.stockStatus" :items="[
      ...
      <UModal v-model="isStoreModalOpen" title="Manage Stores">
          <template #body>
              <div class="space-y-6">
                  <div class="flex items-center gap-2">
                      <UInput v-model="newStoreInput" placeholder="Add new store (e.g. Costco, Walmart)..." class="flex-1" @keyup.enter="addStore" />
                      <UButton label="Add Store" color="primary" variant="soft" @click="addStore" />
                  </div>

                  <div class="space-y-2">
                      <h4 class="text-xs font-bold uppercase tracking-wider text-gray-500">Existing Stores</h4>
                      <div v-for="store in (groceries?.stores || [])" :key="store.id" class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                          <span class="font-medium">{{ store.name }}</span>
                          <UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" @click="deleteStore(store.id)" />
                      </div>
                      <div v-if="(groceries?.stores || []).length === 0" class="text-center py-4 text-gray-400 text-sm italic">
                          No stores added yet.
                      </div>
                  </div>
              </div>
