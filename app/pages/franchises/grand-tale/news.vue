          ...
          <article v-for="(item, index) in newsItems" :key="index" class="group flex flex-col md:flex-row gap-8 items-start border-b border-grand-tale-secondary-800/30 pb-12 last:border-0">
            ...
            <div class="flex-1 space-y-4">
              <div class="flex items-center gap-4 text-xs font-bold uppercase tracking-widest">
                <span class="text-grand-tale-secondary-500">{{ item.category }}</span>
                <span class="text-grand-tale-secondary-100/40">{{ item.date }}</span>
              </div>
              <h2 class="text-2xl font-bold text-white group-hover:text-grand-tale-secondary-400 transition-colors uppercase leading-tight">
                {{ item.title }}
              </h2>
