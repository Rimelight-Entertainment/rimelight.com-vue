...
useSeoMeta({
  ...
  description: "Experience Grand Tale, an epic journey through a shattered world where every choice resonates through eternity.",
});
</script>

<template>
  <div>
    <!-- Hero Section for Grand Tale -->
    <section class="relative min-h-[60vh] flex items-center overflow-hidden border-b border-grand-tale-secondary-800/30">
      ...
      <UContainer class="relative z-10 py-20">
        <div class="max-w-3xl space-y-6">
          <div class="inline-flex items-center gap-2 px-3 py-1 bg-grand-tale-primary-500/10 border border-grand-tale-secondary-500/20 text-grand-tale-secondary-400 text-xs font-bold uppercase tracking-widest">
            Now in Closed Alpha
          </div>
          <h1 class="text-5xl md:text-7xl font-black uppercase tracking-tighter text-white">
            Your Story <br /> <span class="text-grand-tale-secondary-500">Unwritten</span>
          </h1>
          <p class="text-xl text-grand-tale-secondary-100/70 max-w-xl font-light leading-relaxed">
    ...
    <section class="py-20">
      <UContainer>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <UCard v-for="i in 3" :key="i" class="bg-grand-tale-primary-800/20 border-grand-tale-secondary-700/50 hover:border-grand-tale-secondary-500/50 transition-colors">
            <template #header>
              <h3 class="text-xl font-bold text-white uppercase">Feature {{ i }}</h3>
            </template>
            <p class="text-grand-tale-secondary-100/60">
              Discover the deep mechanics and rich lore that make Grand Tale a unique experience in the RPG landscape.
            </p>
          </UCard>
        </div>
      </UContainer>
