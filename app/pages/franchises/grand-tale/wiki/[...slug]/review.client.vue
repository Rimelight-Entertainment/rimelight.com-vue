...
import { convertVersionToPage, syncPageWithDefinition, getLocalizedContent, getPageResolutionPath } from "#rimelight-components/utils"
...
watch([page, versionData], () => {
  if (versionId.value && versionData.value) {
    const converted = convertVersionToPage(versionData.value)

    const typeKey = Object.keys(PAGE_MAP).find(k => k.toLowerCase() === converted.type?.toLowerCase())
    ...
    const typeKey = Object.keys(PAGE_MAP).find(k => k.toLowerCase() === cloned.type?.toLowerCase())
    const definition = typeKey ? (PAGE_MAP as any)[typeKey] : null
...
const handleApprove = async () => {
  ...
  try {
    ...
    toast.add({ color: 'success', title: t('page_version.approved_successfully', 'Version approved') })
    await refreshPage()
    // Go to live page after approval
    await navigateTo(`/franchises/grand-tale/wiki/${slug.value}`)
  } catch (e: any) {
    toast.add({ color: 'error', title: t('page_version.failed_to_approve', 'Failed to approve'), description: e.message })
  } finally {
...
const handleReject = async () => {
  ...
  try {
    ...
    toast.add({ color: 'success', title: t('page_version.rejected_successfully', 'Version rejected') })
    await refreshPage()
    // Go to edit page after rejection
    await navigateTo(`/franchises/grand-tale/wiki/${slug.value}/edit`)
  } catch (e: any) {
    toast.add({ color: 'error', title: t('page_version.failed_to_reject', 'Failed to reject'), description: e.message })
  } finally {
    isActioning.value = null
  }
}
...
const { header: headerStyles, headerGroup, splitContainer, editorColumn, resizer, previewColumn } = reviewStyles()
...
<template>
  <div v-if="pageStatus === 'pending' || (versionId && versionStatus === 'pending')" class="h-full w-full py-20 px-4">
    <USkeleton class="h-12 w-3/4 mb-4" />
    <USkeleton class="h-64 w-full mb-8" />
    <USkeleton class="h-48 w-full" />
  </div>

  <LazyUError v-else-if="pageError || versionError || !localPage" :clear="{ label: 'Back to Wiki' }" :error="{
    status: 404,
        ...
        <template #left>
          <div :class="headerGroup({ class: 'ml-4' })">
            ...
            <span class="text-xs text-neutral-500 dark:text-neutral-400 font-medium whitespace-nowrap">
              Reviewing: <span class="text-neutral-900 dark:text-white">{{ getLocalizedContent(localPage?.title, locale) }}</span>
            </span>
