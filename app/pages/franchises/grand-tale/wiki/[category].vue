...
const category = (route.params.slug as string[])?.[0] || 'characters';
...
const { data: pages, status, error } = await useLazyFetch<Page[]>('/api/pages', {
  query: {
    type: pageType,
    status: 'published',
    limit: 100,
    ...
    <UContainer>
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
        ...
        <main class="lg:col-span-3 space-y-12">
          ...
          <!-- Empty State -->
          <div v-else-if="!pages || pages.length === 0" class="p-12 text-center space-y-4">
