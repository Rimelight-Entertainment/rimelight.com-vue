...
function convertBlock(oldBlock: any): any {
    ...
    if (oldBlock.type === "bullet_list" || oldBlock.type === "ordered_list") {
        ...
        const listItems = oldBlock.slots?.default?.map((li: any) => {
...
async function main() {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const title = typeof oldPage.title === 'string'
                ? { en: oldPage.title }
            ...
            await db.insert(pages).values(newPage as any).onConflictDoUpdate({
                target: pages.id,
                set: newPage as any
            })
