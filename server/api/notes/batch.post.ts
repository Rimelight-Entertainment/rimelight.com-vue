...
const batchActionSchema = v.object({
  ids: v.pipe(v.array(v.string()), v.minLength(1)),
  action: v.picklist(["delete", "archive", "unarchive", "pin", "unpin", "restore", "hard-delete"])
})
...
export default defineEventHandler(async (event) => {
  ...
  const { ids, action } = await readValidatedBody(event, (body) => v.parse(batchActionSchema, body))
