...
export default defineWebSocketHandler({
  open(peer) {
    ...
    const locations = Array.from(peer.peers.values()).map((peer) => parseQuery(peer.websocket.url!))
  ...
  close(peer) {
    ...
    setTimeout(() => {
      const locations = Array.from(peer.peers.values()).map((peer) =>
        parseQuery(peer.websocket.url!)
      )
      peer.publish("visitors", JSON.stringify(locations))
    }, 500)
  }
